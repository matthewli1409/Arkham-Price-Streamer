...
class Candles {
					...
					this.ws.onCandle({
						...
						candles => {
							try {
								...
								const c = candles[1];
								const c_live = candles[0]
								this._updateLiveToDb(ticker, c)
								console.log(ticker, new Date(c_live.mts), c_live.close)
								this._updateLiveToDb(ticker, c_live)
							} catch (err) {
								console.log(err);
							}
						}
	...
	/**
	 * Update candle data to db
	 *
	 * @param {String} ticker Ticker string to be added to database 
	 * @param {object} candle Candle data from BFX, will contain dt, OHLC and volume in some weird order
	 * @private
	 */
	_updateLiveToDb(ticker, candle) {
		db.getDb()
			.collection("Price")
			.updateOne({
				Date_Time: new Date(candle.mts),
